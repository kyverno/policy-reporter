...
func (k *k8sPolicyReportClient) Run(stopper chan struct{}) (*report.Group, error) {
	...
	if !cache.WaitForCacheSync(stopper, polrInformer.HasSynced) {
		return nil, fmt.Errorf("failed to sync policy reports")
	}

	if cpolrInformer != nil && !cache.WaitForCacheSync(stopper, cpolrInformer.HasSynced) {
		return nil, fmt.Errorf("failed to sync cluster policy reports")
	}

	k.synced = true

	return k.debouncer.ReportGroups(), nil
}
...
func NewPolicyReportClient(client versioned.Interface, mapper Mapper, reportFilter report.Filter) report.PolicyReportClient {
	fatcory := externalversions.NewSharedInformerFactory(client, 0)
