...
func Test_SimpleResultMetricGeneration(t *testing.T) {
	gauge := metrics.RegisterSimpleResultGauge("policy_report_simple_result")
	...
	t.Run("Added Metric", func(t *testing.T) {
		handler := metrics.CreateSimpleResultMetricsListener(filter, gauge)
		...
		results := findMetric(metricFam, "policy_report_simple_result")
		if results == nil {
			t.Fatalf("Metric not found: policy_report_simple_result")
		}

		metrics := results.GetMetric()
		if err = testSimpleResultMetricLabels(metrics[0], result2, 1); err != nil {
			t.Error(err)
		}
		if err = testSimpleResultMetricLabels(metrics[1], result1, 1); err != nil {
			t.Error(err)
		}
	})
	...
	t.Run("Modified Metric", func(t *testing.T) {
		...
		metrics := results.GetMetric()
		if err = testSimpleResultMetricLabels(metrics[0], result2, 0); err != nil {
			t.Error(err)
		}
		if err = testSimpleResultMetricLabels(metrics[1], result1, 1); err != nil {
			t.Error(err)
	...
	t.Run("Deleted Metric", func(t *testing.T) {
		...
		if err = testSimpleResultMetricLabels(metrics[1], result1, 0); err != nil {
			t.Error(err)
		}
	})
}

func testSimpleResultMetricLabels(metric *ioprometheusclient.Metric, result report.Result, expVal float64) error {
	...
	if value := *metric.Label[1].Value; value != result.Resource.Namespace {
	...
	if name := *metric.Label[3].Name; name != "severity" {
		return fmt.Errorf("unexpected Name Label: %s", name)
	}
	if value := *metric.Label[3].Value; value != result.Severity {
		return fmt.Errorf("unexpected Severity Label Value: %s", value)
	}

	if name := *metric.Label[4].Name; name != "source" {
		return fmt.Errorf("unexpected Source Label: %s", name)
	}
	if value := *metric.Label[4].Value; value != result.Source {
