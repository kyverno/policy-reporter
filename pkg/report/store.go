...
type PolicyReportStore interface {
	...
	Get(ctx context.Context, id string) (v1alpha1.ReportInterface, error)
	// Add a PolicyReport to the Store
	Add(ctx context.Context, r v1alpha1.ReportInterface) error
	// Update a PolicyReport to the Store
	Update(ctx context.Context, r v1alpha1.ReportInterface) error
...
type policyReportStore struct {
	store map[string]map[string]v1alpha1.ReportInterface
	rwm   *sync.RWMutex
}
...
func (s *policyReportStore) Get(_ context.Context, id string) (v1alpha1.ReportInterface, error) {
	s.rwm.RLock()
...
func (s *policyReportStore) Add(_ context.Context, r v1alpha1.ReportInterface) error {
	s.rwm.Lock()
	defer s.rwm.Unlock()
...
func (s *policyReportStore) Update(_ context.Context, r v1alpha1.ReportInterface) error {
	s.rwm.Lock()
	defer s.rwm.Unlock()
	s.store[GetType(r)][r.GetID()] = r
...
func (s *policyReportStore) CleanUp(_ context.Context) error {
	s.rwm.Lock()
	defer s.rwm.Unlock()
	s.store = map[ResourceType]map[string]v1alpha1.ReportInterface{
		PolicyReportType:        {},
...
func NewPolicyReportStore() PolicyReportStore {
	return &policyReportStore{
		store: map[ResourceType]map[string]v1alpha1.ReportInterface{
			PolicyReportType:        {},
			ClusterPolicyReportType: {},
		},
